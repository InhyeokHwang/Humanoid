...
# -*- coding: utf-8 -*-
"""
read_motor_torque.py

SocketCAN(canX)에서 Robostride 모터들의 현재 토크(N·m)만 읽어 출력.
- 상태 요청(ID type=0x02, 확장프레임)을 직접 날려 응답을 파싱합니다.
- --once 옵션으로 1회만 출력하거나, 기본은 지정한 주기로 반복 출력합니다.

예)
  python3 read_motor_torque.py --ids 1,2,5 --hz 10
  python3 read_motor_torque.py --ids 5 --once
"""
...
def _read_status_once(bus: can.Bus, motor_id: int, master_id: int = MASTER_ID, timeout: float = 0.2) -> Optional[Dict[str, float]]:
    ...
    while time.time() < t_end:
        ...
        return {'torque_nm': torque_nm}

    return None

def get_torque_nm(bus: can.Bus, motor_id: int, master_id: int = MASTER_ID, timeout: float = 0.2) -> Optional[float]:
    st = _read_status_once(bus, motor_id, master_id, timeout)
    return None if st is None else st['torque_nm']
...
def main():
    ap = argparse.ArgumentParser(description="Robostride 모터 토크 리더")
    ap.add_argument("--channel", default=DEFAULT_CAN_CHANNEL, help="SocketCAN 채널명 (기본: can1)")
    ...
    try:
        ...
        print(" - canX가 UP 상태인지 확인:  sudo ip link set can1 up type can bitrate 1000000  # (채널명/비트레이트 맞춰서)")
    ...
    def read_and_print_once():
        ...
        line_parts = [f"[{ts}]"]
        for mid in ids:
            tq = get_torque_nm(bus, mid, master_id=args.master, timeout=args.timeout)
            if tq is None:
                line_parts.append(f"ID{mid}: N/A")
            else:
                line_parts.append(f"ID{mid}: {tq:+.3f} N·m")
